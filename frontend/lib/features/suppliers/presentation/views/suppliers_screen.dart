...
class _SuppliersScreenState extends ConsumerState<SuppliersScreen> {
  ...
  Widget build(BuildContext context) {
    final supplierState = ref.watch(suppliersListProvider);
    ...
    return MainLayout(
      ...
      child: supplierState.when(
        ...
        data: (suppliers) {
          final reversedSuppliers = suppliers.reversed.toList();
          final filteredSuppliers = reversedSuppliers.where((s) {
            final q = _searchCtrl.text.toLowerCase();
            return s.name.toLowerCase().contains(q) ||
                   (s.phone?.toLowerCase().contains(q) ?? false);
          }).toList();

          final limit = _pageSize == 'All' ? filteredSuppliers.length : int.tryParse(_pageSize) ?? 10;
          final paginatedSuppliers = filteredSuppliers.take(limit).toList();

          double totalPurchase = 0;
          for (var s in paginatedSuppliers) {
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 8),
                            _navyBtn(Icons.file_upload_outlined, 'Import', onTap: () {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Importing suppliers...'), duration: Duration(seconds: 1)),
                              );
                            }),
                            const SizedBox(width: 8),
                            _navyBtn(Icons.file_download_outlined, 'Template', onTap: () {
                              ScaffoldMessenger.of(context).showSnackBar(
                            ...
                            PopupMenuButton<String>(
                              ...
                              onSelected: (val) async {
                                if (val == 'edit') {
                                  ...
                                  if (confirm == true) {
                                    await ref.read(suppliersListProvider.notifier).removeSupplier(r.id);
                                  }
                                }
                              },
                            ),
                      ...
                      // Footer
                      _buildFooter(filteredSuppliers.length, paginatedSuppliers.length),
                    ],
                  ),
                ),
              ],
  ...
  Widget _navyBtn(IconData icon, String label, {required VoidCallback onTap}) {
    return InkWell(
      onTap: onTap,
